...
export default function CandidateDashboard() {
  ...
  const [stats, setStats] = useState<any>({}); // FIXED: Initialized as object instead of array
  ...
  // FIXED: Extracted fetch logic so we can call it again after saving
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push("/login"); // FIXED: Use Next.js router
      return;
    }
    ...
    try {
      ...
      fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/applications/candidate/${decoded.userId}`, {
        ...
        .then((res) => res.json())
        .then((data) => {
          if (Array.isArray(data)) setApplications(data); // FIXED: Protective check
        })
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 overflow-x-hidden">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6 px-4 md:px-10 lg:px-20 py-15">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 transition-colors duration-300 w-full">
          ...
          <ul className="space-y-4">
            {/* FIXED: Added optional chaining (?) to prevent crash if applications is undefined */}
      ...
      {/* FIXED: Passed onSaveSuccess to trigger data refetch */}
      <ProfileModal 
        isOpen={isOpen} 
